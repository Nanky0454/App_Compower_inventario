...
function fillAddForm(row) {
  ...
  document.getElementById("add-tipo").value = row.tipo;
  document.getElementById("add-sede").value = row.sede;
  document.getElementById("add-codigo").value = row.codigo ?? "";
  document.getElementById("add-nombre").value = row.nombre;
  document.getElementById("add-valor-unitario").value =
...
function fillEditForm(row) {
  ...
  document.getElementById("edit-valor_unitario").value =
    row.valor_unitario ?? 0;
}
...
export async function submitEditForm(e) {
  ...
  if (!codigo) errorMsg = "El código es obligatorio.";
    ...
    errorMsg = "Valor unitario inválido.";
  if (errorMsg) {
...
export async function submitAddForm(e) {
  e.preventDefault();
  const form = e.target;
  // Validaciones
  ...
  if (!codigo) errorMsg = "El código es obligatorio.";
    ...
    errorMsg = "Cantidad inválida.";
  if (errorMsg) {
    showModalError("add-modal", errorMsg);
    return;
  }
  const data = {
    id: form["add-id"].value,
    tipo: form["add-tipo"].value,
    sede: form["add-sede"].value,
    codigo,
    cantidad,
    valor_unitario,
    nombre,
  ...
  const res = await fetch("/api/inventario/crud", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ action: "actualizar", ...data }),
  });
  if (res.ok) {
    showModalSuccess("add-modal", "¡Agregado correctamente!");
    ...
    const errText = await res.text();
    showModalError("add-modal", errText || "Error al agregar");
  }
}
